...
function Section({ label, children }) {
  ...
  return (
    <div className="mb-6">
      <div className="font-sans text-xs tracking-widest uppercase mb-2" style={{ color: "#5ED29C" }}>
        {label}
      </div>
      {children}
    </div>
...
export default function ProblemStatement({ problem }) {
  ...
  return (
    <div style={{ border: "1px solid #17201C", background: "#070B0A" }}>
      <style>{`
        .cf-md { color: #FFFFFF; font-family: 'Inter', system-ui, sans-serif; font-size: 0.9rem; line-height: 1.7; }
        .cf-md p { margin: 0 0 0.7rem; }
        .cf-md ul, .cf-md ol { margin: 0.4rem 0 0.8rem 1.25rem; }
        .cf-md li { margin-bottom: 0.25rem; }
        .cf-md strong { color: #ECF3EF; font-weight: 700; }
        .cf-code { font-family: 'Spline Sans Mono Variable', ui-monospace, monospace; font-size: 0.82em; background: #111917; color: #5ED29C; padding: 0.1em 0.35em; border-radius: 3px; border: 1px solid #242424; }
        .cf-pre { font-family: 'Spline Sans Mono Variable', ui-monospace, monospace; font-size: 0.78rem; line-height: 1.6; background: #080808; border: 1px solid #17201C; padding: 0.85rem 1rem; overflow-x: auto; margin: 0 0 0.8rem; color: #FFFFFF; }
      `}</style>
      ...
      <div className="px-6 lg:px-8 py-6">
        {story && (
          <div
            ...
            style={{ borderLeft: "2px solid #5ED29C", background: "#5ED29C08", fontStyle: "italic" }}
          >
            <MD>{story}</MD>
          </div>
        )}
        ...
        {Array.isArray(examples) && examples.length > 0 && (
          <Section label="Examples">
            <div className="space-y-4">
              {examples.map((ex, i) => (
                <div key={i} style={{ border: "1px solid #17201C" }}>
                  <div className="grid grid-cols-2" style={{ borderBottom: ex.explanation ? "1px solid #17201C" : "none" }}>
                    <div className="px-4 py-3" style={{ borderRight: "1px solid #17201C" }}>
                    ...
                    <div className="px-4 py-3">
                      ...
                      <pre className="font-mono text-xs whitespace-pre-wrap" style={{ color: "#5ED29C", margin: 0 }}>{ex.output}</pre>
                    </div>
                  </div>
                  {ex.explanation && (
