      ...
      >
        ...
        <div className="flex items-center gap-4 flex-shrink-0">
          {brief.estimatedHours != null && (
            <span className="hidden sm:flex items-center gap-1.5 font-sans text-xs" style={{ color: "#FFFFFF" }}>
              <Clock size={12} /> ~{brief.estimatedHours}h
            </span>
          )}
          <ChevronDown size={16} style={{ color: "#FFFFFF", transform: open ? "rotate(180deg)" : "none", transition: "transform .2s" }} />
        </div>
      </button>
              ...
              <ol className="space-y-2">
                {brief.milestones?.map((m, i) => (
                  <li key={i} className="flex gap-3">
                    ...
                    <div>
                      <div className="font-display text-sm font-medium" style={{ color: "#FFFFFF" }}>{m.title}</div>
                      <div className="font-display text-xs" style={{ color: "#FFFFFF" }}>{m.detail}</div>
                    </div>
                  </li>
                ))}
              </ol>
            ...
            <Section icon={<Award size={13} />} label="Done well means" accent="#f0c000">
              <ul className="space-y-2">
                {brief.rubric?.map((r, i) => (
                  <li key={i}>
                    <div className="font-display text-sm font-medium" style={{ color: "#FFFFFF" }}>{r.criterion}</div>
                    <div className="font-display text-xs" style={{ color: "#FFFFFF" }}>{r.description}</div>
                  </li>
                ))}
              </ul>
            </Section>
