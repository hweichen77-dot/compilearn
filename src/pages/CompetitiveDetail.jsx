    ...
    <div style={{ background: "#15130E", minHeight: "100vh" }}>
      ...
      <div className="relative pt-20" style={{ borderBottom: "1px solid #262219" }}>
        ...
        <div className="max-w-5xl mx-auto px-8 lg:px-16 py-10">
          <Link to={createPageUrl("Competitive")} className="font-mono text-xs tracking-widest uppercase mb-8 inline-block" style={{ color: "#C9C1B2" }}>
            ← Competitive
          </Link>
          <div className="flex items-start gap-5">
            <span className="font-mono font-bold flex-shrink-0" style={{ fontSize: "3.5rem", lineHeight: 1, color: "#ECE7DC", letterSpacing: "-0.05em" }}>
              {DIFF_NUM[problem.difficulty] || "01"}
            </span>
            <div>
              <div className="flex items-center gap-3 mb-2 flex-wrap">
                <span className="font-mono text-xs tracking-widest uppercase px-2.5 py-1" style={{ color: "#E8A33C", border: "1px solid #E8A33C33", background: "#E8A33C10" }}>
                  {problem.difficulty}
                </span>
                <span className="font-mono text-xs" style={{ color: "#C9C1B2" }}>{problem.algorithm_focus}</span>
              </div>
              ...
              <div className="flex items-center gap-4 mt-3 font-mono text-xs" style={{ color: "#8F8779" }}>
      ...
      <div className="max-w-5xl mx-auto px-8 lg:px-16 py-10 space-y-8">
        ...
        <AnimatePresence>
          {passed && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex items-center gap-4 px-6 py-5" style={{ border: "1px solid #E8A33C33", background: "#E8A33C08" }}>
              ...
              <div>
                <div className="font-mono text-xs tracking-widest uppercase mb-1" style={{ color: "#E8A33C" }}>Accepted</div>
        ...
        <div className="flex flex-wrap gap-3">
          {problem.editorial && (
            <button onClick={() => setShowEditorial(!showEditorial)} className="font-mono text-xs tracking-widest uppercase px-4 py-2.5 transition-all duration-150" style={{ color: showEditorial ? "#E8A33C" : "#BBB3A4", border: `1px solid ${showEditorial ? "#E8A33C33" : "#262219"}`, background: showEditorial ? "#E8A33C10" : "transparent" }}>
              {showEditorial ? "— Editorial" : "+ Editorial"}
            </button>
          )}
          {problem.solution_cpp && (
            <button onClick={() => setShowSolution(!showSolution)} className="font-mono text-xs tracking-widest uppercase px-4 py-2.5 transition-all duration-150" style={{ color: "#ECE7DC", border: "1px solid #262219" }}>
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
              <div style={{ border: "1px solid #262219", background: "#131009" }}>
                <div className="px-5 py-3" style={{ borderBottom: "1px solid #262219" }}>
                  <span className="font-mono text-xs tracking-widest uppercase" style={{ color: "#E8A33C" }}>Editorial</span>
                </div>
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
              <div style={{ border: "1px solid #262219", background: "#131009" }}>
                <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: "1px solid #262219" }}>
                  ...
                  <span className="font-mono text-xs px-2 py-0.5" style={{ color: "#E8A33C", border: "1px solid #E8A33C33", background: "#E8A33C10" }}>C++</span>
                </div>
