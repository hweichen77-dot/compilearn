  ...
  "lessons": [
    {
      ...
      "hints": [
        "A count arrives, then that many point values, and one number comes back: all of them added together.",
        "You need a running total established before any traversal, a traversal that visits every value exactly once, and a hand back of the total at the end rather than a display of it.",
        "set sum to 0, add every value in the array to sum, then ____"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many numbers, and one number comes back: how many of them split into groups of three with nothing left over. Zero counts, since it divides evenly.",
        "You need a counter established before the traversal, a traversal of every value, a remainder test that decides whether the counter moves, and a hand back of the counter.",
        "set count to 0, for each value in the array raise count when ____, then return count"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A width and a height arrive, and two numbers come back: the area on the first line and the perimeter on the second. Both answers are computed from state the object remembers, not from values passed in at call time.",
        "You need two hidden fields, a constructor that copies both incoming values into them, and two no argument behaviors that read those fields.",
        "declare two private int fields, write a constructor that ____, then add area returning width times height and perimeter returning twice their sum"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many numbers, at least one of them. One number comes back: the largest. The values may all be negative.",
        "You need a best so far value that starts as a real element rather than an invented number, a traversal of the remaining values, and a replacement whenever a larger value appears.",
        "set max to ____, then for every value in the array replace max when that value is greater, and return max"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A row count and a column count arrive, then that many numbers filling a grid row by row. One number comes back: every cell added together.",
        "You need a running total, an outer traversal that picks a row, and an inner traversal that walks the columns of that row, with each bound read from the grid itself rather than from the input numbers.",
        "set sum to 0, then for each row index r and each column index c inside that row add ____ to sum, and return sum"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A single number arrives and the total of every whole number from one up to it comes back. When the arriving number is below one the total is zero.",
        "You need a running total established before the loop, a counted traversal whose stopping test includes the arriving number itself, and a hand back of the total.",
        "set sum to 0, run i from 1 while ____, adding i to sum on each pass, then return sum"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many words, then one more word to look for. One number comes back: how many of the earlier words carry the same text as the target. Two separately read words with identical letters must count as a match.",
        "You need a counter, a traversal of every word, and a content comparison rather than an identity comparison, since separately read words are separate objects even when their letters agree.",
        "set count to 0, for each word in the array raise count when ____ the target, then return count"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many scores, which the object stores for itself. One number comes back: how many of those scores reach sixty. A score of exactly sixty is one of them.",
        "You need a counter established before the traversal, a traversal of the field the object already holds, a threshold test that includes the edge value, and a hand back of the counter.",
        "set count to 0, for each stored score raise count when the score ____ 60, then return count"
      ],
