  ...
  "lessons": [
    {
      ...
      "hints": [
        "One type name arrives as text. One word goes out: whether a variable of that type stores the value itself or an arrow to something living elsewhere.",
        "A comparison of the incoming text against each of the three primitive names, joined so any single match counts, wrapped in a two-way decision. Comparing text is a method call, not an operator.",
        "Skeleton: String type = sc.next(); if (____) { println \"primitive\" } else { println \"object\" }. The blank is three text comparisons joined so one match is enough."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One name arrives as text. One line goes out: the word Hello, a comma and a space, the name, then an exclamation mark.",
        "One String variable holding the name and one expression that joins fixed text on both sides of it before anything prints.",
        "Skeleton: String name = sc.next(); String greeting = ____; println greeting. The blank joins three pieces, and the punctuation lives inside the quoted pieces."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One word arrives. One line goes out: the character at the very front, a space, then the character at the very end. A one-letter word prints that same letter twice.",
        "The count of characters in the word, then two slices of it, one anchored at the front and one anchored at the position just before the end.",
        "Skeleton: read word; int len = word.length(); String first = word.substring(0, 1); String last = word.substring(____); println first + \" \" + last. The blank is the index the final character sits at."
      ],
    ...
    },
    {
      ...
      "hints": [
        "A full line of text arrives. What goes out is everything before the first space. When the line holds no space at all, the whole line goes out unchanged.",
        "One search for the position of a space, one test of whether that search found anything, and a slice running from the start of the line up to that position.",
        "Skeleton: String full = sc.nextLine(); int space = full.indexOf(\" \"); if (____) { println full } else { println full.substring(0, space) }. The blank is the test for the not-found signal."
      ],
    ...
    },
    {
      ...
      "hints": [
        "Two words arrive. They go out on one line separated by a space, with the one that comes first in the dictionary printed first. Two identical words look the same either way round.",
        "One comparison that reports ordering rather than a yes or no, and a two-way decision that prints the pair in one arrangement or the other. The sign of that comparison is what matters, not its size.",
        "Skeleton: read a and b; if (____) { println a + \" \" + b } else { println b + \" \" + a }. The blank compares the ordering result against 0."
      ],
    ...
    },
    {
      ...
      "hints": [
        "Two whole numbers arrive, the two short sides of a right triangle. One whole number goes out: the long side, with any fractional part dropped.",
        "Each side squared, the two squares added, the square root of that sum, then a conversion from the decimal the root hands back into a whole number.",
        "Skeleton: read a and b as ints; double h = ____; println (int) h. The blank is the root of the sum of the two squares."
      ],
    ...
    },
    {
      ...
      "hints": [
        "Two numbers arrive, but they arrive as text. One number goes out: what the two of them add up to as numbers, not as text stuck together.",
        "Two String variables converted into ints by a static method on the wrapper class, then added. Without that conversion the + operator joins instead of adding.",
        "Skeleton: read first and second as Strings; int a = Integer.parseInt(first); int b = ____; println a + b. The blank repeats the same conversion on the second token."
      ],
    ...
    },
    {
      ...
      "hints": [
        "An array of whole numbers is handed to a method. Every number in it has to end up twice what it was, and that change has to still be there when main prints the array.",
        "A loop that walks every position of the array and writes a new value back into that same position. The method returns nothing, so the array it was given is the only way out.",
        "Skeleton: inside doubleAll, for (int i = 0; i < arr.length; i++) { ____; }. The blank writes into position i rather than into a local variable."
      ],
