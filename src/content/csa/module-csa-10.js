  ...
  "lessons": [
    {
      ...
      "hints": [
        "A single whole number arrives and its running product down to one comes back. Zero and one both answer one, and every larger number answers itself times the answer for the number just below it.",
        "You need a case that returns without calling itself, reached when the number is one or smaller, and a case that calls itself on something smaller, here the number reduced by one.",
        "if n is one or less return 1, otherwise return n times ____"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A single number arrives and the numbers from one up to it come back, each on its own line, smallest first. No loop is allowed, so the increasing order has to come from the order in which calls finish, not the order in which they start.",
        "You need a case that returns without calling itself once the number reaches zero, and a case that calls itself on the number below before producing any output, so the output happens while the calls unwind.",
        "if n is zero return, otherwise ____ and then print n"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A position in the sequence arrives and the value at that position comes back. Position zero answers zero, position one answers one, and every later position answers the sum of the two positions before it.",
        "You need cases that return without calling themselves for the two smallest positions, and a case that calls itself twice on smaller positions and combines the two answers by addition.",
        "if n is below 2 return n, otherwise return the answer for n minus 1 ____ the answer for n minus 2"
      ],
    ...
    },
    {
      ...
      "hints": [
        "One word arrives and the same characters come back in the opposite order. The first character of the input has to finish as the last character of the output.",
        "You need a case that returns without calling itself when the text is empty or a single character, and a case that calls itself on the text with its first character removed, then rejoins that first character on the far end.",
        "if the length is one or less return the text, otherwise return the reversed rest ____ the first character"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many numbers, and their total comes back. The array itself never shrinks while the total is being built.",
        "You need a position marker that moves forward one step per call, a case that returns without calling itself once the marker has passed the last element, and a case that adds the element at the marker to the answer for everything after it.",
        "if i equals the array length return 0, otherwise return the element at i plus ____"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A sorted run of numbers arrives along with a target, and one number comes back: the position holding the target, or negative one when it is absent. Most of the numbers are never examined at all.",
        "You need a low and a high bound, a case that returns without calling itself when the bounds have crossed and another when the middle value is the target, and a case that calls itself on whichever half can still contain the target with the middle position left out.",
        "if low is above high return -1, set mid to the average of low and high, return mid when it holds the target, and otherwise return the search over ____ so the range strictly shrinks"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many numbers, and the same numbers come back in increasing order on one line. Duplicates survive, so nothing is dropped along the way.",
        "You need a case that returns without calling itself for a piece of length zero or one, a split into two halves, a call on each half, and a combining step that repeatedly takes the smaller of the two front values.",
        "if the length is one or less return it, otherwise split at the midpoint and return the combination of the sorted left half and ____"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many numbers, and the same numbers come back in increasing order on one line. The values are rearranged inside the original array rather than copied into a new one.",
        "You need a case that returns without calling itself when only one element is in range, a case that calls itself on the range one element shorter, and a placement step that runs after that call returns, sliding larger values right to open a gap.",
        "if n is one or less return, otherwise ____, then hold the last element, shift every larger earlier element one slot right, and drop the held value into the gap"
      ],
