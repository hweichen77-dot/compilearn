  ...
  "lessons": [
    {
      ...
      "hints": [
        "Lines keep arriving until the input runs out, and you have no way to know in advance how many there will be. The only output is how many there were.",
        "You need a collection that grows as each line arrives, because no fixed size can be chosen up front, plus a loop that keeps reading while input remains.",
        "make an empty list of strings\nwhile another line is available:\n    read the line\n    if the line is not empty:\n        ____\nprint how many the list holds"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many words, then one more number naming a position among those words. The output is the single word sitting at that position, counting from zero.",
        "You need a list built in input order so that position in the list matches position in the input, followed by a single positional read.",
    ...
    },
    {
      ...
      "hints": [
        "A run of numbers arrives. The first becomes zero, the last disappears entirely, and everything else keeps its value and its relative order.",
        "You need one in place replacement that leaves the count untouched, then one deletion aimed at the final position that shrinks the count by one.",
        "build the list from input\nreplace the value at position 0 with 0\nremove the element at position ____\nprint the remaining values separated by single spaces"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many numbers. Two numbers come out: their total, and that total split evenly across the count with any remainder thrown away.",
        "You need a list of boxed integers, an accumulator that unwraps each element as it adds it, and a division that deliberately keeps no fractional part.",
        "sum = 0\nloop i from 0 while i < the list size:\n    sum = sum + the element at i\nprint sum\nprint ____"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A run of words arrives and one comes out: the one with the most characters. When two words tie, the one that arrived first wins.",
        "You need a best so far word and a loop that hands you each word directly, since position never matters. The comparison is on character counts, not on the words themselves.",
        "longest = the empty string\nfor each word in the list:\n    if ____:\n        longest = that word\nprint longest"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A run of numbers arrives. Every value divisible by two disappears, the rest keep their original relative order, and the result may end up empty.",
        "You need a traversal that survives the list shrinking underneath it. Either walk from the last position toward the first, or walk forward and only advance when nothing was deleted on that pass.",
        "build the list from input\n____:\n    if the element at position i is even:\n        remove the element at position i\nprint the remaining values separated by single spaces"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A run of numbers arrives and two come out: the largest value, and the position where that value first appeared. Ties belong to the earlier position.",
        "You need two variables moving together, both seeded from the very first element, and both updated only when a later element genuinely beats the current best rather than merely matching it.",
        "max = the first element\nidx = 0\nloop i from 1 while i < the list size:\n    if ____:\n        max = the element at i\n        idx = i\nprint max\nprint idx"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A run of numbers arrives and the same numbers come out in ascending order, duplicates included. Nothing is added or deleted, only rearranged.",
        "You need a sorted region at the front that grows by one element per pass. Each new element slides left past everything larger than it, which means overwriting positions rather than inserting.",
        "loop i from 1 while i < the list size:\n    key = the element at i\n    j = i - 1\n    while j is at least 0 and the element at j is greater than key:\n        copy the element at j into position j + 1\n        j = j - 1\n    ____\nprint every value separated by single spaces"
      ],
