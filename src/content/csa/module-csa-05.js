  ...
  "lessons": [
    {
      ...
      "hints": [
        "You have one name arriving on input and one line of output that puts that name after a fixed greeting. The name has to be handed to a Person and then come back out as finished text.",
        "You need an actual object built from the Person blueprint, and a behavior on that class that returns the greeting as a value instead of printing it. The constructor already stores the name, so the missing piece is the behavior that reads it back.",
        "main:\n  read the name\n  build a Person from that name\n  print what that Person's greeting gives back\n\nPerson greeting behavior:\n  return ____"
      ],
    ...
    },
    {
      ...
      "hints": [
        "The first number tells you how many scores follow. Every score has to be folded into one running total that survives from one score to the next, and only the final total is printed.",
        "You need a loop that runs exactly as many times as the first number said, feeding one value per pass into the object, plus a piece of object state that persists between those calls.",
        "main:\n  read n\n  build one Counter\n  repeat n times:\n      read the next value and hand it to the Counter\n  print the Counter's total\n\naddPoints(p):\n  ____"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Two numbers arrive at the moment the object is created, and they must still be there later when the total is computed. Nothing sets them after creation.",
        "You need the special member that runs during creation: named exactly like the class, no return type, taking the two values and copying them into the two fields.",
        "class Time:\n  fields hours and minutes\n\n  ____(int h, int m):\n      hours = h\n      minutes = m"
      ],
    ...
    },
    {
      ...
      "hints": [
        "One number arrives and one number comes out. The output equals the input, except an input below the low limit reports the low limit and an input above the high limit reports the high limit.",
        "You need a three way decision inside the mutator, made before anything is stored, so the field never holds a value outside the allowed range.",
        "setVolume(v):\n  if v is below the low limit:\n      store the low limit\n  else if ____:\n      store the high limit\n  else:\n      store v"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Two numbers arrive carrying the same names as the two pieces of state they are supposed to fill. Both pieces of state must hold those numbers by the time the distance is computed.",
        "Inside the constructor the parameters hide the fields of the same name, so each assignment needs a way to say explicitly that the left side means the object's own field.",
        "Point(int x, int y):\n  ____.x = x\n  ____.y = y"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Two numbers arrive: a starting balance and a requested amount. The output balance either dropped by exactly that amount or did not move at all.",
        "You need a single guard inside the withdraw behavior that has to hold before the subtraction runs, and it has two parts, one about the amount itself and one about the funds available.",
    ...
    },
    {
      ...
      "hints": [
        "The program creates some number of objects and throws every reference away. The number created still has to be reportable afterwards, so the tally cannot live inside any one object.",
        "You need a tally that belongs to the class rather than to an instance, bumped once during each creation, and a reporting behavior that can be called without any object at hand.",
        "class Widget:\n  ____ int count = 0\n\n  constructor:\n      count = count + 1\n\n  ____ int getCount():\n      return count"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Two numbers live inside the object, and printing the object itself has to produce those two numbers joined by a slash instead of the default cryptic text.",
        "You need the exact method Java reaches for whenever an object is printed. Its name, visibility, return type, and empty parameter list all have to match, or Java ignores it.",
        "class Fraction:\n  fields num and den\n\n  ____ {\n      return num + \"/\" + den\n  }"
      ],
