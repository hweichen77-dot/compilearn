  ...
  "lessons": [
    {
      ...
      "hints": [
        "The first number says how many values follow. Every one of those values has to be stored before any of them is printed, and each prints on its own line in the order it arrived.",
        "You need a fixed size block of int slots whose size comes from that first number, then one pass to fill it and a second pass to print it.",
        "read n\n____\nloop i from 0 while i < n:\n    store the next input value at position i\nloop i from 0 while i < n:\n    print the value at position i"
      ],
    ...
    },
    {
      ...
      "hints": [
        "The first and last values trade places. Everything between them keeps its value and its position, and the whole sequence prints on one line separated by single spaces.",
        "You need a holding variable so the first value is not destroyed when the last one is copied over it, plus the position of the final slot expressed from the length.",
        "temp = the value at position 0\nthe value at position 0 = the value at the last position\nthe value at the last position = ____\nprint every value separated by single spaces"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Some count of numbers arrives and two numbers come out: how many there were, and what they add up to.",
        "You need a running total started before the traversal begins, and the array's own count of slots rather than a number you kept from the input.",
        "sum = 0\nloop i from 0 while ____:\n    sum = sum + the value at position i\nprint the array's own length\nprint sum"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A run of numbers arrives and one number comes out: how many of them divide by two with nothing left over. Where a value sits never matters.",
        "You need a counter starting at zero and a loop that hands you each value directly, since no position is ever needed. The test itself is about the remainder after dividing by two.",
    ...
    },
    {
      ...
      "hints": [
        "A run of numbers arrives and one comes out: the single largest. Every value could be negative, so the answer is not guaranteed to be above zero.",
        "You need a running best so far that starts from a real element of the array rather than a made up number, then one comparison against each remaining element.",
        "max = ____\nloop i from 1 while i < length:\n    if the value at i is greater than max:\n        max = the value at i\nprint max"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A run of numbers arrives followed by one target. The output is a position, or a value that could never be a position when the target is absent.",
        "You need a result variable pre loaded with the not found signal, and a scan that records the position and stops the moment the first match appears.",
        "found = ____\nloop i from 0 while i < length:\n    if the value at i equals the target:\n        found = i\n        stop scanning\nprint found"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A run of numbers arrives and one comes out: how many times a value is immediately repeated by the value directly after it. Three equal values in a row make two such pairs.",
        "You need a comparison that reads one slot past the current one, which means the traversal has to stop one position earlier than a normal full sweep.",
        "count = 0\nloop i from 0 while ____:\n    if the value at i equals the value at i + 1:\n        count = count + 1\nprint count"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A run of numbers arrives and one comes out: how many of them sit strictly above the mean of the whole run. Nothing can be decided about any value until every value has been seen.",
        "You need two separate traversals, one to build the total and one to compare against the mean, and the mean has to keep its fractional part rather than being truncated.",
        "sum = 0\nfor each value: sum = sum + value\naverage = ____\ncount = 0\nfor each value:\n    if value is strictly greater than average:\n        count = count + 1\nprint count"
      ],
