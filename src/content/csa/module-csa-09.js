  ...
  "lessons": [
    {
      ...
      "hints": [
        "A single name arrives on one line, and one greeting line goes back out containing that exact name. The greeting wording is written in only one place, even though the object producing it is of a more specialized kind.",
        "You need a general class that owns the greeting behavior, a specialized class declared as a kind of that general class so it receives the behavior without restating it, and an object of the specialized kind created in main.",
        "give the general class a greeting behavior that prints Hello, then the name; declare the specialized class as ____ the general one; then in main create the specialized object and ask it to greet"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A name arrives on one line and a grade number on the next, and one line comes back combining both. The finished object holds two pieces of state that were declared at two different levels of the hierarchy.",
        "You need the general class to keep owning the name and storing it, the specialized class to declare only the grade, and the specialized constructor to hand the name upward before it touches anything of its own.",
        "in the specialized constructor take a name and a grade, first ____, then store the grade in this object's own field, and add a reader that returns the grade"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Two objects exist, one general and one specialized, and each reports a different sentence about itself. The general object's sentence must stay exactly as it was after the specialized one gets its own.",
        "You need the specialized class to supply a second version of the same named behavior, matching the original in every detail the compiler uses to identify a method, so the specialized object picks up the new sentence.",
        "in the specialized class declare a describe behavior whose ____ matches the inherited one exactly, and return the cat sentence from its body"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A name and a department arrive on two lines, and one line comes back that opens with the general employee text and then continues with the department. The first half of that output is text this class never writes itself.",
        "You need the specialized class to replace the info behavior, but the replacement has to ask the general version for its half of the answer rather than rebuilding it from the same data.",
        "in the manager's info behavior return ____ joined with a comma, the word Dept, a colon, and the department field"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Three objects of three different kinds sit together in one collection, and each reports its own sound in order, one per line. The loop that prints them never learns which kind it is holding.",
        "You need one array whose declared element type is the most general of the three, holding one object of each kind, and a single traversal that asks every element the same question.",
        "make an array whose declared element type is ____, fill it with one dog, one cat, and one plain animal in that order, then loop over it printing the sound of each element"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Two numbers arrive and one line comes back showing them wrapped in parentheses, separated by a comma and a space. The printing statement in main is already written and it is handed the object itself, never any text.",
        "You need the class to supply its own version of the standard text description that printing relies on, and that version has to hand the text back rather than display it.",
        "in the point class declare the standard description behavior with its exact required signature and ____ the text built from an open parenthesis, x, a comma and a space, y, and a close parenthesis"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Four numbers arrive and become two separate points. One word comes back, true or false, saying whether the two points hold the same pair of coordinates even though they are two distinct objects.",
        "You need the class to define sameness by value: a guard that rejects anything that is not a point, a conversion so the other object's coordinates become reachable, and a comparison of both numbers.",
        "declare the equality behavior taking the most general object type, return false unless the argument ____, then convert it and return whether both coordinate pairs match"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many width and height pairs, and one number comes back: every rectangle's area added together. The loop doing the adding is written against the general shape type, not the rectangle type.",
        "You need the rectangle class to supply its own area behavior in place of the general one, plus an accumulator and a traversal of the general typed array that asks each element for its area.",
        "in the rectangle class replace the general area behavior with one returning width times height, then set total to 0, add ____ to total for each shape in the array, and print total"
      ],
