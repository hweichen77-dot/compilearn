  ...
  "lessons": [
    {
      ...
      "hints": [
        "Row count and column count arrive first, then that many values in row order, then two more numbers naming one cell. The output is the single value living in that cell.",
        "You need a grid whose first dimension is the row count, filled by nested loops in the same order the values arrive, then one read using both indices.",
        "read rows and cols, make the grid\nloop r over the rows:\n    loop c over the columns:\n        store the next input value at ____\nread r and c\nprint the cell at row r, column c"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Every value in the grid comes out on one line: row 0 left to right, then row 1, and so on. Single spaces between values and none at either end.",
        "You need nested loops where the outer one walks rows and the inner one walks columns, with both bounds taken from the grid itself rather than from the input numbers.",
        "loop r from 0 while r < ____:\n    loop c from 0 while c < the column count of row r:\n        print a separating space unless this is the very first value\n        print the cell at row r, column c\nprint a newline"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Every cell contributes to one number. Values can be negative, and each cell must be counted exactly once.",
        "You need one accumulator declared before both loops so it survives the entire sweep, plus nested loops that reach every cell exactly once.",
        "total = 0\nloop r over the rows:\n    loop c over the columns of row r:\n        ____\nprint total"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Every cell is compared against one target value and the answer is how many of them matched. Where the matches sit never matters.",
        "You need two nested for each loops. The outer one hands you a whole row, which is itself a one dimensional array, and the inner one hands you a single cell value.",
        "count = 0\nfor each ____ in the grid:\n    for each value in that row:\n        if the value equals the target:\n            count = count + 1\nprint count"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Each row collapses to a single number, its total, and the answer is the biggest of those totals. A row can total to a negative number.",
        "You need a per row accumulator that starts over at the top of every row, plus a best so far that lives outside both loops and never resets.",
        "best = a value no row total could beat\nloop r over the rows:\n    ____\n    loop c over the columns of row r:\n        sum = sum + the cell at row r, column c\n    if sum is greater than best:\n        best = sum\nprint best"
      ],
    ...
    },
    {
      ...
      "hints": [
        "Each vertical slice collapses to a single number and the answer is the biggest of those. A slice is one fixed column read down through every row.",
        "You need the same per slice accumulator pattern as rows, but with the loop nesting reversed, because a column is not stored as its own array and has to be gathered cell by cell.",
        "best = a value no column total could beat\n____:\n    sum = 0\n    loop r from 0 while r < the row count:\n        sum = sum + the cell at row r, column c\n    if sum is greater than best:\n        best = sum\nprint best"
      ],
    ...
    },
    {
      ...
      "hints": [
        "A grid arrives followed by one target. The output is two numbers naming where the target first appears, scanning the top row first and left to right within a row, or a single value meaning it never appeared.",
        "You need indexed nested loops, because the answer is a position rather than a value, plus a way to stop scanning once a match is recorded and to tell afterwards that nothing was found.",
        "foundR = -1, foundC = -1\nloop r over the rows while nothing has been found yet:\n    loop c over the columns of row r:\n        if the cell at row r, column c equals the target:\n            ____\nif foundR is still -1:\n    print -1\nelse:\n    print foundR then foundC"
      ],
    ...
    },
    {
      ...
      "hints": [
        "The value at row r, column c of the input has to end up at row c, column r of the output. A grid with R rows and C columns produces one with C rows and R columns.",
        "You need a second grid sized with the two dimensions exchanged, filled by reading the source in row order and writing each value into its swapped position.",
        "make a result grid sized [the column count][the row count]\nloop r over the source rows:\n    loop c over the source columns:\n        ____\nprint the result one row per line, values separated by single spaces"
      ],
