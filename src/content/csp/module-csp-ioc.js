  ...
  "lessons": [
    {
      ...
      "hints": [
        "A count arrives, then that many words, each one labelling an effect as helpful or harmful. One phrase comes out saying which side outweighs the other, and an exact tie has its own answer.",
        "You need a tally of each of the two labels across the whole list, then a three-way comparison of those tallies rather than a simple bigger-or-not test.",
        "count how many labels say beneficial and how many say harmful, then print the verdict chosen by ____, remembering the case where the two tallies are equal."
      ],
    ...
    },
    {
      ...
      "hints": [
        "Two whole numbers arrive, the share of urban households online and the share of rural households online. One number comes out, how far apart those two shares sit, and it is never negative.",
        "You need both figures as whole numbers and the size of the difference between them, independent of which of the two is larger.",
        "read both numbers, subtract one from the other, then print ____ of that difference."
      ],
    ...
    },
    {
      ...
      "hints": [
        "Four whole numbers arrive: an approved count and a total for one group, then the same pair for another. One word comes out saying whether the two approval shares sit far enough apart to be worth flagging.",
        "You need each group's share measured against that group's own total rather than against the combined total, then the size of the distance between the two shares held against a fixed cutoff.",
        "work out each group's rate as its approvals divided by its own total, then if ____ is larger than the cutoff print biased, otherwise print fair."
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many whole-number ratings. One number comes out, the middle-of-the-road value across all of them, shown to two decimal places.",
        "You need the ratings as numbers rather than text, their combined total, and that total shared out over how many ratings there were, then trimmed to two places.",
        "add all the ratings together, divide that total by ____, then print the result rounded to two decimal places."
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many words, each recording one access as permitted or not permitted. One word comes out, and a single bad record is enough to spoil the whole log.",
        "You need every record checked against one condition, with a verdict that holds only when the condition holds for all of them, not merely for most of them.",
        "look at the records that follow the count, then if ____ record says authorized print compliant, otherwise print violation."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One license label arrives as a line of text. One phrase comes out saying whether the work may be reused straight away or whether the creator has to be asked first.",
        "You need the label cleaned of the whitespace around it, then tested for membership in a small fixed set of labels that permit reuse.",
        "read the label and trim it, then if it is ____ print that reuse is allowed, otherwise print that permission is needed."
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many field names. One number comes out, how many of those names belong to a known list of fields that can identify a person.",
        "You need the fixed list of identifying field names to test against, and a tally across the given names of how many of them appear in that list.",
        "hold the identifying names in one fixed collection, then count the given names for which ____, then print that count."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One line of text arrives, a password. One word comes out describing its strength, and that word rests on three separate yes-or-no properties of the text.",
        "You need a score that starts at zero and rises by one for each property that holds: enough characters, at least one digit somewhere, and at least one character that is neither a letter nor a digit.",
        "start the score at zero, add one for the length, one if any character is a digit, one if any character is ____, then turn the score into strong, medium, or weak."
      ],
