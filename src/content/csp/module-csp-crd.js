  ...
  "lessons": [
    {
      ...
      "hints": [
        "One line of numbers arrives, separated by spaces. One number comes out, all of them added together, and the line may hold only a single value.",
        "You need the line broken into separate pieces, and a running total that already exists before the first piece is looked at.",
        "break the line into pieces, set the total to zero, then for each piece add ____ into the total, then print the total."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One line of numbers arrives. One number comes out, the biggest among them, and a line holding a single number answers with that number.",
        "You need a running best that starts as an actual value from the line, plus one pass across the remaining values comparing each against it.",
        "seed the best with the first value, then for each remaining value if ____ make it the new best, then print the best."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One line of numbers arrives. One number comes out, the total of only those that split into pairs with nothing left over, and a line with no such values answers zero.",
        "You need a running total set before the walk begins, plus a test on each value that decides whether that value joins the total at all.",
        "set the total to zero, then for each value if ____ add it into the total, then print the total."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One whole price arrives and one whole price comes out, cut by a rough tenth. Small prices can come back unchanged, because their tenth rounds away to nothing.",
        "You need a tenth of the price with the fractional part dropped rather than rounded, then that amount taken off the original price.",
        "read the price as a whole number, then print the price minus ____."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One whole number arrives. One of two words comes out, decided by whether that number reaches a cutoff of sixty. Sixty itself belongs on the passing side.",
        "You need one comparison against the cutoff, written so the cutoff value itself counts as passing, and a two-way choice built on that comparison.",
        "read the score, then if ____ print the passing word, otherwise print the failing word."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One line of numbers arrives. One number comes out, how many of them sit strictly above zero. Zero itself does not qualify.",
        "You need a counter that exists before the walk begins, and a test on each value that raises the counter only when the value clears zero.",
        "set the count to zero, then for each value if ____ raise the count by one, then print the count."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One line arrives, and it may hold numbers or nothing at all. One number comes out, their whole-number average, and an empty line answers with zero.",
        "You need a count of how many values there really are, examined before any division happens, so the case of no values never reaches the division.",
        "break the line into pieces, then if there are no pieces print 0, otherwise print the total divided by ____ with the fraction dropped."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One line of scores arrives. Two lines come out: their whole-number average, then one of two words decided by whether that average reaches sixty.",
        "You need a running total across every score, a count of how many scores there were, and one comparison of the resulting average against the cutoff.",
        "add every score into a total, print the total divided by the count with the fraction dropped, then if ____ print the passing word otherwise the failing one."
      ],
