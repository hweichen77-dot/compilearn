  ...
  "lessons": [
    {
      ...
      "inline_quizzes": [
        {
          ...
          "correct_index": 1,
          "explanation": "DNS works like a phone book, converting human-friendly names such as compilearn.com into the numeric IP addresses computers use to route data."
        }
      ],
      "quiz_questions": [
        {
      ...
      "animated_diagrams": [
        {
          "title": "Loading compilearn.com",
          "caption": "A name becomes an address, then a request travels out and a response comes back.",
          "loop": false,
          "nodes": [
            { "label": "You type a name", "sub": "compilearn.com", "detail": "Humans use names, but computers route by numeric IP addresses." },
      ...
      "hints": [
        "One line of text arrives holding an address. One word comes out saying whether that text follows the four-number pattern, and a single bad piece spoils the whole address.",
        "You need the text broken into its dot-separated pieces, a count of those pieces, and a check on every piece covering both the characters it holds and the value it stands for.",
        "split the line on dots, then if there are not exactly four pieces call it invalid, otherwise for each piece if ____ call it invalid, then print the verdict once at the end."
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many lines, each holding a position number and a word. One line comes out holding the words in their true order, separated by single spaces.",
        "You need each line broken into its position and its word, the two kept together as a pair, and the whole collection put in order by position before any joining happens.",
        "read each line as a position and a word, keep the pairs together, order the pairs by ____, then join the words with single spaces."
      ],
    ...
    },
    {
      ...
      "hints": [
        "Counts of routers and links arrive, then the links themselves, then a source and a destination. One number comes out, the fewest links a packet crosses, or -1 when no route exists.",
        "You need a lookup from each router to its direct neighbors with both directions recorded, a record of the distance to every router reached so far, and a waiting line that hands back the router reached earliest.",
        "record each link in both directions, set the source distance to zero and put it in the line, then while the line holds anything take the front router and for each unrecorded neighbor set its distance to ____ and add it to the line."
      ],
    ...
    },
    {
      ...
      "hints": [
        "Routers and links arrive, then a source, a destination, and one router that has gone offline. One word comes out saying whether traffic can still get through without touching the broken one.",
        "You need the same neighbor lookup as before, a record of what has been visited, a rule that treats the broken router as if it were absent from the network, and a separate answer when an endpoint is the broken router itself.",
        "if the source or the destination is the broken router say unreachable, otherwise walk out from the source stepping onto a neighbor only when ____, then report whether the destination was reached."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One line arrives holding a start-up delay in milliseconds, a rate in megabits per second, and a file size in megabytes. One whole number comes out, the total milliseconds until the whole file has arrived.",
        "You need the size expressed in the same unit family the rate uses, a streaming duration converted into milliseconds, and the start-up delay added on top before any rounding.",
        "turn the megabytes into megabits by ____, divide that by the rate, scale the result to milliseconds, add the delay, then print the rounded whole number."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One line of text arrives. Either two of its pieces come back out, the action and the resource, or a fixed refusal message when the line does not fit the expected shape.",
        "You need the line broken on spaces, a count of the pieces, a fixed set of allowed actions to test the first piece against, and a check on how the last piece begins.",
        "split the line, then if there are exactly three pieces and the first is in the allowed set and the last ____, print the first two pieces, otherwise print the refusal."
      ],
    ...
    },
    {
      ...
      "hints": [
        "Task durations arrive along with a number of workers. Two numbers come out: how long one worker alone would take, and how long the crowd takes counted until the last one finishes.",
        "You need a running load for every worker starting at zero, the tasks considered from the largest down, and each one added to whichever load is currently the smallest.",
        "start every worker at zero, then for each task from largest to smallest add it to ____, then print the total of the durations and the largest worker load."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One line arrives holding the share of the work that can be split up and how many processors are available. One number comes out, how many times faster the split version runs, shown to two decimal places.",
        "You need the share that cannot be split alongside the share that can, the splittable share spread across the processors, and those two amounts added together underneath a one.",
        "take one minus the parallel share, add the parallel share divided by the processor count, then print one divided by that sum shown to ____."
      ],
