  ...
  "lessons": [
    {
      ...
      "hints": [
        "Two whole numbers arrive on one line. One of two words comes out, telling you whether both of those numbers sit above zero. Zero itself does not count as above zero.",
        "You need two separate comparisons against zero, and a way to fold their two answers into one answer that only holds when neither comparison failed.",
        "read both numbers, then print the result of the first sitting above zero ____ the second sitting above zero."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One whole number from zero to one hundred arrives. One letter comes out, picked from four bands, and each cutoff value belongs to the higher band.",
        "You need a chain of tests examined in a fixed order where only the first match takes effect, plus a final catch-all for everything below the lowest cutoff.",
        "read the score, then if it reaches 90 print A, else if ____ print B, else if it reaches 70 print C, otherwise print F."
      ],
    ...
    },
    {
      ...
      "hints": [
        "An age arrives, then a yes or no word. One number comes out, the price. Anyone under thirteen pays the same low price no matter what the word says.",
        "You need one outer two-way split on the age, and a second two-way split that lives entirely inside one side of the first, so only non-children ever reach it.",
        "if the age is under 13 set the price to 5, otherwise if ____ set it to 8 and if not set it to 12, then print the price."
      ],
    ...
    },
    {
      ...
      "hints": [
        "One whole number of at least one arrives. One number comes out, the count of transformations needed to drive it down to one, and a starting value of one needs none at all.",
        "You need a repetition governed by a stopping condition rather than a fixed count, a two-way choice inside it, and a counter raised once per transformation.",
        "set the count to zero, then while ____ halve the value if it is even or otherwise triple it and add one, raising the count each pass, then print the count."
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many numbers on one line. One number comes out, how many of those values split into pairs with nothing left over.",
        "You need a counter that already exists before the walk begins, one pass across every value, and a test that raises the counter only when it holds.",
        "set the count to zero, then for each value if ____ raise the count by one, then print the count once the walk is over."
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many values on one line, then a target. One number comes out, the position of the earliest copy of the target, or a marker meaning it never appeared.",
        "You need a result that starts at a value no real position could equal, a walk that knows positions rather than just values, and a halt the moment the first match shows up.",
        "set the result to -1, then walk the positions in order and if the value there matches the target record the position and ____, then print the result."
      ],
    ...
    },
    {
      ...
      "hints": [
        "A count arrives, then that many values already in increasing order, then a target. One number comes out, the position of the target, or a marker meaning it is absent.",
        "You need two bounds fencing off the stretch of data still worth checking, and every pass has to discard everything on one side of the middle.",
        "while the low bound has not passed the high one, take the middle and stop if it matches, otherwise ____ so the half that cannot hold the target is thrown away, then print the result."
      ],
    ...
    },
    {
      ...
      "hints": [
        "A line of step letters arrives. One number comes out, where the walker ends up once every step has been applied in order, and it can be negative.",
        "You need one piece of state holding the current position, set before any step is read, plus a two-way choice inside a walk over every step.",
        "set the position to zero, then for each step ____ if it is a left step and add one otherwise, then print the position."
      ],
