const project = {
  ...
  description:
    "A warm-up before the build track. Learn the loop every AI product follows, set up Python and your API key once, pick the starter tech stack the projects use, and see how each guide is laid out. By the end nothing in the track feels unfamiliar.",
  difficulty: "beginner",
...
const lessons = [
  {
    ...
    key_terms: [
      ...
      { term: "The model as a function", definition: "The mental model that treats an AI call as one capable but unreliable function inside an ordinary program." },
    ],
    animated_diagrams: [
      {
    ...
    hints: [
      "Nothing gets built here. What comes out is a labelled map of one AI product you have already used: what goes in, what the model gets asked, what comes back, how you would know it is good, and where the result ends up.",
      "You need the six named stages written in order, one sentence of description beside each, and a mark on the single stage that is carrying the unusual work in that particular product.",
      "list the six steps down the page, write beside each one what it does in the product you picked, then mark ____ as the step that makes this product different from the rest",
    ],
  },
  {
    id: "prod-00-2",
    ...
    hints: [
      "The artifact is one small Python file that proves your key reached the program. Nothing is sent to a model yet. The only thing printed is evidence that a secret loaded.",
      "You need a value that lives in your shell rather than in the file, a read of that value by its name, and an early stop with a clear message when the read comes back empty.",
      "read the environment variable named AI_API_KEY into a variable, exit with a message if it is empty, then print ____ instead of the key itself",
    ],
  },
  {
    id: "prod-00-7",
    ...
    illustrative: true,
    explanation:
      "The fastest way to stall is to spend a week choosing tools. So here is one stack that is beginner-friendly and enough to build every project in this track. It is the exact stack the build projects use, so once you have it working, project one runs with no surprises. You can swap any single piece later without relearning the rest.\n\n## The stack, one line each\n\n- **Language: Python.** Every guide here is Python. It has the least ceremony and the most examples online.\n- **Model API: Claude (Anthropic).** Every build project calls Claude through Anthropic's official library. You get an API key from console.anthropic.com and put it in `ANTHROPIC_API_KEY`.\n- **Talking to the model: the `anthropic` library.** One small library, one client, and you are calling a real model.\n- **UI (optional): Streamlit.** When you want a web page instead of a terminal, Streamlit turns a Python file into a shareable app with no HTML.\n- **Storage: plain files, then SQLite.** A JSON file is enough for most projects. The search and memory projects later use SQLite, which ships with Python.\n- **Hosting (optional): Streamlit Community Cloud.** Free, and it deploys straight from a GitHub repo when you want a live link.\n\n## Get the model call working\n\nCreate an API key at console.anthropic.com, then set it and install the library:\n\n```bash\nexport ANTHROPIC_API_KEY=\"your-key-here\"\npip install anthropic\n```\n\nThen this is a complete, working AI program, and it is exactly the shape every build project uses:\n\n```python\nimport os\nimport anthropic\n\nclient = anthropic.Anthropic(api_key=os.environ[\"ANTHROPIC_API_KEY\"])\n\nresp = client.messages.create(\n    model=\"claude-sonnet-4-6\",\n    max_tokens=200,\n    messages=[{\"role\": \"user\", \"content\": \"Say hello in one short sentence.\"}],\n)\nprint(resp.content[0].text)\n```\n\nRun it. If a sentence prints, your whole stack works. That `client` and that `messages.create` call are the one unusual function from lesson 1. Everything else you build wraps around it.\n\n## No credit card yet? A free path to start\n\nAnthropic needs a payment method. If you want to try the ideas for free first, a provider like Groq gives you a capable model with no card, using the OpenAI-compatible format:\n\n```python\nimport os\nfrom openai import OpenAI   # pip install openai\n\nclient = OpenAI(\n    api_key=os.environ[\"GROQ_API_KEY\"],\n    base_url=\"https://api.groq.com/openai/v1\",\n)\nresp = client.chat.completions.create(\n    model=\"llama-3.3-70b-versatile\",\n    messages=[{\"role\": \"user\", \"content\": \"Say hello.\"}],\n)\nprint(resp.choices[0].message.content)\n```\n\nThe idea is identical, only the client and the reply shape differ. The build projects are written against Claude, so switch to the Anthropic snippet above when you are ready to follow them line by line.\n\n## Adding a UI later, in about five lines\n\nWhen a project is worth a web page, you don't need a front-end framework:\n\n```python\nimport streamlit as st\n\nprompt = st.text_input(\"Ask me anything\")\nif prompt:\n    st.write(ask_model(prompt))   # your loop from earlier\n```\n\n`streamlit run app.py` opens it in a browser. That's the entire UI story for this track.\n\n## What usually goes wrong\n\n| What you see | What caused it | How to fix it |\n| --- | --- | --- |\n| An authentication error on the very first call | The key is missing from this terminal, or it is stored under a name your code does not read | Export ANTHROPIC_API_KEY in the same terminal, then check the spelling against the line that reads it |\n| KeyError: ANTHROPIC_API_KEY before any request leaves your machine | os.environ raises the moment a name is not set | Export it first, or read it with os.environ.get so you can print your own message |\n| The free Groq snippet reports that the model does not exist | A Claude model name got pasted into the OpenAI-compatible client | Model names belong to their provider, so keep each snippet with its own model name |\n| ModuleNotFoundError for anthropic right after installing it | The install landed in a different Python than the one running the file | Install with python -m pip install anthropic |\n\n## Why this specific stack\n\nEvery piece is popular enough that errors are one search away, and none of them lock you in. Claude is what the projects teach against, Python is the most documented language, and Streamlit skips the hardest part of web apps. Pick it, stop deliberating, and spend your energy on the actual product.",
    key_terms: [
      { term: "Anthropic SDK", definition: "The official `anthropic` Python library. You create a client with your key, then call `client.messages.create` to talk to Claude." },
      { term: "ANTHROPIC_API_KEY", definition: "The environment variable every build project reads your Claude key from, so your key stays out of your code." },
    ...
    animated_diagrams: [
      {
        ...
        caption: "Your code sits in the middle. Claude does the thinking, and an optional UI shows the result.",
        nodes: [
          { label: "You (Python)", sub: "your program", detail: "Your Python file holds the loop: read input, build the prompt, handle the reply." },
          { label: "anthropic lib", sub: "the messenger", detail: "The library packages your prompt and sends it to Claude over HTTP." },
          { label: "Claude API", sub: "the model", detail: "Claude runs the model and sends a reply back that your code reads from resp.content." },
          { label: "Streamlit", sub: "optional UI", detail: "When you want a web page, Streamlit shows the result without any HTML." },
        ],
      },
    ],
    callouts: [
      { type: "tip", position: "after", title: "you can swap any one piece", content: "Want to start free without a card? Use the Groq snippet above. Prefer a plain terminal? Skip Streamlit. The stack is a default, not a cage. The point is to stop choosing and start building, then match the Anthropic setup when you begin the projects." },
    ],
    inline_quizzes: [
      {
        question: "Which setup do the build projects in this track expect?",
        options: ["The openai library pointed at Groq", "The anthropic library with ANTHROPIC_API_KEY", "No API key at all", "A different provider for each project"],
        correct_index: 1,
        explanation: "Every build project calls Claude through the anthropic library and reads your key from ANTHROPIC_API_KEY. Groq is only a free way to try the ideas first.",
      },
    ],
    hints: [
      "The artifact is the smallest complete AI program: one file that sends a sentence to a model and prints what comes back. A sentence appearing in your terminal is proof the whole stack works.",
      "You need a client built from a key that is read out of the environment, one request carrying a model name and a single user message, and one read of the text off the reply.",
      "build the client from the key in your environment, call messages.create with a model, a token limit, and one user message, then print ____",
    ],
  },
  {
    id: "prod-00-3",
    ...
    hints: [
      "Nothing gets coded here. What you walk away with is a habit: open any lesson in this track and you can say which stage it belongs to and what runnable thing exists by the end of that stage.",
      "You need the four stages of a guide in order, the five parts that live inside a single lesson, and a rule for what you do once you have been stuck for more than a few minutes.",
      "open any project guide, name the stage each of its eight lessons belongs to, then decide your first move when a lesson breaks, which is to read ____",
    ],
  },
  {
    id: "prod-00-4",
    ...
    hints: [
      "The artifact is a block of reusable text: the standing rules written once, plus a slot for the one thing that differs on every run. Whatever it produces has to be regular enough for a parser to read without guessing.",
      "You need two separate messages, one holding the role, the job, the constraints, and the exact return shape, the other holding only this run's input, and a small function that pairs them.",
      "write the standing rules as a template holding the role, the job, the constraints, and the exact return format, then build the message pair where the only thing that changes per call is ____",
    ],
  },
  {
    id: "prod-00-5",
    ...
    hints: [
      "The artifact is a wrapper around a single model call that either hands back clean structured data or fails on purpose. It has to survive a dropped network, a chatty reply, and an input you never tested.",
      "You need a bounded loop over attempts, a parse that tolerates extra text sitting around the object, a short pause between tries, and a rule for when to stop trying and let the error escape.",
      "loop a fixed number of attempts, call the model, cut the text down to the span between the first brace and the last brace, parse that, and on the final failed attempt ____",
    ],
  },
  {
    id: "prod-00-6",
    ...
    hints: [
      "Nothing new gets coded. What comes out is a judgement about a project you already built, plus the small record that goes with it on your Portfolio shelf.",
      "You need the three conditions that define done in this track, and for each finished project a one-line description, a saved example run, and a link when the project produced one.",
      "for the project you just finished, run it from a clean folder with one command, feed it empty input to see whether it survives, hand your instructions to someone else, then save ____ alongside the code",
    ],
  },
];
